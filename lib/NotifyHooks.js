...
class NotifyHooks extends EventEmitter {
    ...
    console.log('parsed', upcomingJson, 'is array:', Array.isArray(upcomingJson))
  ...
  onShowTimer(show) {
    ...
    let channels = show.show.channels.map(channelInfo => channelInfo.key)

    _.each(hooks, (hooksUrls, channelKey) => {
      if (_.contains(channels, channelKey)) {
  ...
  _createTimer(show, delay) {
    ...
    let showTimer = setTimeout(this.onShowTimer, notificationDelay, show)
    let timerObj = {
      id: show.id,
      timer: showTimer
    }
    ...
    hooksUrls.forEach(url => {
      var options = {
        ...
        body: {
            ...
            avatar_url: 'http://buznik.net/di/avatar.png'
        },
        json: true
      };
